...
#### 1.3.1 至少一个指数为奇数：拆出一次项凑微分

若 $m+n$ 为奇数，则 $m,n$ 中恰有一个为奇数，因此一定可以使用奇次凑微分。更一般地，只要 $m,n$ 中至少有一个为奇数，就能从对应的奇次幂中拆出一个一次因子，将它并入微分；剩余的偶次幂再通过
...
化为另一个三角函数的多项式。因此，积分最终都可以化为单变量多项式积分。

特别地，当 $m,n$ 均为奇数时，虽然 $m+n$ 为偶数，但仍可任选一个奇次幂拆出一次因子。因此，“$m+n$ 为奇数”是可以凑微分的充分条件，而不是必要条件。

- 若 $m$ 为奇数，拆出一个 $\sin x$，令 $u=\cos x$：

$$
\begin{aligned}
I_{m,n}
&=\int \sin x\,(\sin^2x)^{\frac{m-1}{2}}\cos^n x\,dx\\
&=-\int (1-u^2)^{\frac{m-1}{2}}u^n\,du.
\end{aligned}
$$

- 若 $n$ 为奇数，拆出一个 $\cos x$，令 $u=\sin x$：

$$
\begin{aligned}
I_{m,n}
&=\int \cos x\,\sin^m x(\cos^2x)^{\frac{n-1}{2}}\,dx\\
&=\int u^m(1-u^2)^{\frac{n-1}{2}}\,du.
\end{aligned}
$$
...
&=-\int (u^2-u^4)\,du\qquad (u=\cos x)\\
&=-\frac{\cos^3x}{3}+\frac{\cos^5x}{5}+C.
\end{aligned}
$$

这里的核心不是分部积分，而是：**拆出一次因子凑微分，再用平方恒等式消去另一个变量。**

#### 1.3.2 两个指数均为偶数：反复降幂

若 $m,n$ 均为偶数，则不能直接拆出单个 $\sin x$ 或 $\cos x$ 来凑微分。此时使用降幂公式
...
把幂次减半。若展开后仍有高次幂，则继续降幂；出现不同频率三角函数的乘积时，再使用积化和差公式。经过有限次变换后，被积函数会化为常数项与若干 $\sin(kx)$、$\cos(kx)$ 的线性组合，从而逐项积分。
...
$$
\sin^{2k}x=\left(\frac{1-\cos2x}{2}\right)^k,
\qquad
\cos^{2k}x=\left(\frac{1+\cos2x}{2}\right)^k,
$$

展开后继续对其中的 $\cos^j2x$ 降幂。例如：

$$
\begin{aligned}
\sin^4x
&=\left(\frac{1-\cos2x}{2}\right)^2\\
&=\frac{3}{8}-\frac12\cos2x+\frac18\cos4x,
\end{aligned}
$$
...
$$
\int\sin^4x\,dx
=\frac{3x}{8}-\frac14\sin2x+\frac1{32}\sin4x+C.
$$
...
\boxed{
\begin{array}{ll}
m\text{ 或 }n\text{ 至少一个为奇数}
&\Longrightarrow \text{从奇次幂拆出一次项，凑微分并化为多项式；}\\[4pt]
m,n\text{ 均为偶数}
&\Longrightarrow \text{使用半角公式反复降幂，再逐项积分。}
\end{array}
}
$$
