# 微分算子法求解线性常系数非齐次微分方程

## 1. 方法概述

**微分算子法**（Differential Operator Method），又称为**运算子法**，是一种系统求解**线性常系数非齐次微分方程**的方法。该方法尤其适用于右端函数为指数函数、多项式、三角函数，或其线性组合的情形。

设 $D = \tfrac{d}{dx}$，则微分方程可表示为

$$
L(D)y = f(x),
$$

其中 $L(D)$ 为关于 $D$ 的多项式，称为**线性常系数微分算子**，$f(x)$ 为已知非齐次项。


## 2. 基本原理

解的一般形式为

$$
y(x) = y_h(x) + y_p(x),
$$

其中：

- $y_h(x)$：对应齐次方程 $L(D)y = 0$ 的通解；
- $y_p(x)$：对应非齐次方程 $L(D)y = f(x)$ 的一个特解。

利用算子符号，可以形式化地写为

$$
y_p(x) = \frac{1}{L(D)}f(x).
$$

具体计算时，需要根据 $f(x)$ 的类型选择合适的展开或代换方法。


## 3. 常见右端函数的处理方法

### 3.1 指数函数型 $f(x) = e^{\lambda x}$

利用性质

$$
D^n e^{\lambda x} = \lambda^n e^{\lambda x}
\quad \Rightarrow \quad
L(D)e^{\lambda x} = L(\lambda)e^{\lambda x},
$$

可得：

- 若 $L(\lambda) \neq 0$，则
  $$
  y_p = \frac{1}{L(\lambda)}e^{\lambda x}.
  $$

- 若 $L(\lambda) = 0$，即 $e^{\lambda x}$ 属于齐次解，则需改为
  $$
  y_p = x^k \cdot \frac{1}{L^{(k)}(\lambda)}e^{\lambda x}, \quad 
  k = \text{零点的重数}.
  $$



### 3.2 多项式型 $f(x) = P_n(x)$

若 $L(D)$ 无零根（即常数项 $a_0 \neq 0$），则特解 $y_p$ 仍为 $n$ 次多项式：

$$
y_p(x) = A_n x^n + \cdots + A_1 x + A_0,
$$

将其代入方程即可确定系数。  
若 $L(D)$ 有零根，则需在 $y_p(x)$ 前乘以 $x^k$，以保证解的线性无关性。



### 3.3 三角函数型 $f(x) = \sin(\omega x)$ 或 $\cos(\omega x)$

由于

$$
D^2 \sin(\omega x) = -\omega^2 \sin(\omega x),
$$

算子作用可转化为代入 $D^2 = -\omega^2$。  
对于含有奇次算子的情况，可通过引入共轭因子，将其转化为偶次算子。例如：

$$
\begin{aligned}
\frac{1}{D^2 + 2D + 3}\cos x 
&= \frac{1}{2D + 2}\cos x \\[6pt]
&= \tfrac{1}{2}\,\frac{D - 1}{(D+1)(D-1)}\cos x \\[6pt]
&= \tfrac{1}{2}\,\frac{D+1}{D^2 - 1}\cos x \\[6pt]
&= -\tfrac{1}{4}(D+1)\cos x \\[6pt]
&= -\tfrac{1}{4}\bigl(-\sin x + \cos x\bigr).
\end{aligned}
$$

另一种方法是采用指数形式：

$$
L(D)e^{i\omega x} = L(i\omega)e^{i\omega x}.
$$

因此：

- 若 $L(-\omega^2) \neq 0$，则
  $$
  y_p = \frac{1}{L(-\omega^2)}\sin(\omega x) 
  \quad \text{或} \quad 
  y_p = \frac{1}{L(-\omega^2)}\cos(\omega x).
  $$

- 若 $L(-\omega^2) = 0$，即对应于特征方程的根，则应在解中引入 $x^k$ 因子，以确保与齐次解线性无关。



### 3.4 混合型：$e^{\lambda x}P(x)$ 或 $e^{\lambda x}\sin(\omega x)$

处理原则为**因子分离 + 代入 $D \to D+\lambda$**：

- 若 $f(x) = e^{\lambda x}P(x)$，
  $$
  \frac{1}{L(D)}e^{\lambda x}P(x)
  = e^{\lambda x}\cdot \frac{1}{L(D+\lambda)}P(x).
  $$

- 若 $f(x) = e^{\lambda x}\sin(\omega x)$，
  $$
  y_p = e^{\lambda x}\cdot \frac{1}{L(D+\lambda)}\sin(\omega x).
  $$



## 4. 核心微分算子恒等式的证明

设

$$
L(D)=a_0+a_1D+\cdots+a_mD^m.
$$

下面只给出各性质成立的核心步骤。

### 4.1 指数函数的特征函数性质

因为

$$
D^j e^{\lambda x}=\lambda^j e^{\lambda x},
$$

所以

$$
L(D)e^{\lambda x}
=\sum_{j=0}^m a_jD^j e^{\lambda x}
=\sum_{j=0}^m a_j\lambda^j e^{\lambda x}
=L(\lambda)e^{\lambda x}.
$$

当 $L(\lambda)\neq0$ 时，取 $y_p=L(\lambda)^{-1}e^{\lambda x}$ 代回，即得

$$
\frac{1}{L(D)}e^{\lambda x}=\frac{1}{L(\lambda)}e^{\lambda x}.
$$

### 4.2 指数移位定理

对任意充分可微的函数 $u(x)$，由乘积求导法则，

$$
D\bigl(e^{\lambda x}u\bigr)
=e^{\lambda x}(D+\lambda)u.
$$

反复作用 $D$，得到

$$
D^j\bigl(e^{\lambda x}u\bigr)
=e^{\lambda x}(D+\lambda)^j u,
$$

从而

$$
L(D)\bigl(e^{\lambda x}u\bigr)
=e^{\lambda x}L(D+\lambda)u.
$$

令 $L(D+\lambda)u=P_n(x)$，便有

$$
\frac{1}{L(D)}\left[e^{\lambda x}P_n(x)\right]
=e^{\lambda x}\frac{1}{L(D+\lambda)}P_n(x).
$$

### 4.3 多项式上的有限截断

若 $P_n(x)$ 的次数不超过 $n$，则

$$
D^{n+1}P_n(x)=0.
$$

当 $L(0)\neq0$ 时，可在 $D=0$ 附近将 $1/L(D)$ 形式展开为

$$
\frac{1}{L(D)}=c_0+c_1D+c_2D^2+\cdots.
$$

它作用于 $P_n$ 后，所有高于 $n$ 次的微分项都为零，因此实际计算自动截断：

$$
\frac{1}{L(D)}P_n
=\sum_{j=0}^{n}c_jD^jP_n.
$$

### 4.4 三角函数的复指数处理

由欧拉公式

$$
e^{i\omega x}=\cos(\omega x)+i\sin(\omega x)
$$

及指数函数的特征函数性质，有

$$
L(D)e^{i\omega x}=L(i\omega)e^{i\omega x}.
$$

当 $L(D)$ 的系数为实数时，算子作用与取实部、虚部可以交换。因此，求出复指数右端对应的特解后，分别取实部或虚部，就得到余弦或正弦右端对应的特解。将 $i\omega$ 换成 $\lambda+i\omega$，同理可处理 $e^{\lambda x}\cos(\omega x)$ 与 $e^{\lambda x}\sin(\omega x)$。

### 4.5 共振时乘 $x^k$ 的原因

若 $\lambda$ 是 $L(r)$ 的 $k$ 重零点，则

$$
L(r)=(r-\lambda)^kM(r),\qquad M(\lambda)\neq0.
$$

由指数移位定理，

$$
L(D)\bigl(e^{\lambda x}u\bigr)
=e^{\lambda x}D^kM(D+\lambda)u.
$$

右端出现 $D^k$，所以需要使 $u$ 含有 $x^k$，令第 $k$ 次求导后产生非零项。这就是共振时将通常的试探形式乘以 $x^k$ 的根本原因。

## 5. 总结表格

设常系数线性非齐次微分方程为

\[
L(D)y=f(x),\qquad D=\frac{\mathrm d}{\mathrm dx},
\]

其中 \(L(r)\) 为对应的特征多项式。令 \(k\) 表示相应复数是 \(L(r)\) 的零点重数；若不是零点，则取 \(k=0\)。

| 非齐次项 \(f(x)\) | 核心算子恒等式（计算依据） | 特解的试探形式 \(y_p\) |
|---|---|---|
| \(e^{\lambda x}\) | \(L(D)e^{\lambda x}=L(\lambda)e^{\lambda x}\)。当 \(L(\lambda)\neq 0\) 时，\(\displaystyle \frac1{L(D)}e^{\lambda x}=\frac1{L(\lambda)}e^{\lambda x}\) | 若 \(L(\lambda)\neq0\)，则 \(\displaystyle y_p=\frac{1}{L(\lambda)}e^{\lambda x}\)；若 \(\lambda\) 是 \(L\) 的 \(k\) 重零点，则取 \(y_p=Cx^ke^{\lambda x}\) |
| \(P_n(x)\) | \(D^{n+1}P_n(x)=0\)，因此将 \(\displaystyle \frac1{L(D)}\) 按 \(D\) 展开后作用于 \(P_n(x)\) 时，级数会有限截断 | \(\displaystyle y_p=x^kQ_n(x)\)，其中 \(Q_n(x)\) 是次数不超过 \(n\) 的待定系数多项式，\(k\) 为 \(0\) 作为 \(L\) 的零点重数 |
| \(\sin(\omega x)\) 或 \(\cos(\omega x)\) | \(L(D)e^{i\omega x}=L(i\omega)e^{i\omega x}\)，求得复数形式后取虚部或实部 | \(\displaystyle y_p=x^k\bigl(A\cos(\omega x)+B\sin(\omega x)\bigr)\)，其中 \(k\) 为 \(i\omega\) 作为 \(L\) 的零点重数 |
| \(e^{\lambda x}P_n(x)\) | \(\displaystyle \frac1{L(D)}\!\left[e^{\lambda x}P_n(x)\right]=e^{\lambda x}\frac1{L(D+\lambda)}P_n(x)\)（指数移位定理） | \(\displaystyle y_p=x^ke^{\lambda x}Q_n(x)\)，其中 \(k\) 为 \(\lambda\) 作为 \(L\) 的零点重数 |
| \(e^{\lambda x}\sin(\omega x)\) 或 \(e^{\lambda x}\cos(\omega x)\) | \(L(D)e^{(\lambda+i\omega)x}=L(\lambda+i\omega)e^{(\lambda+i\omega)x}\)，求得复数形式后取虚部或实部 | \(\displaystyle y_p=x^ke^{\lambda x}\bigl(A\cos(\omega x)+B\sin(\omega x)\bigr)\)，其中 \(k\) 为 \(\lambda+i\omega\) 作为 \(L\) 的零点重数 |

> **说明：** 表中的“乘 \(x^k\)”就是共振修正规则。这里的 \(k\) 不是任意选择的，而是非齐次项所对应特征值在特征多项式 \(L(r)\) 中的重数；不存在共振时 \(k=0\)。对于实系数方程，非实特征根总是成共轭对出现。

## 6. 例题解析

**例题**：求微分方程的特解：

$$
y'' - 3y' + 2y = e^{x}
$$

令：

$$
L(D) = D^2 - 3D + 2
$$

右端为 $e^x$，代入得：

$$
L(D)y_p = e^x \Rightarrow y_p = \frac{1}{L(1)}e^x = \frac{1}{1 - 3 + 2}e^x = \frac{1}{0}e^x
$$

**失败！** 说明 $D=1$ 是特征根（即 $e^x$ 是齐次解），应改用：

$$
y_p = x \cdot \frac{1}{L'(1)}e^x = x \cdot \frac{1}{(2D - 3)'|_{D=1}}e^x = x e^x
$$


1. e.g.1：$y'' + y = \sin x$
2. e.g.2：$y'' - 4y' + 4y = x^2+3x +6$
3. e.g.3：$y'' + y' +3y = xe^x$
4. e.g.4：$y'' + 2y' + 5y = e^{2x}\cos x$

## 例题求解

**e.g.1：$\sin(\omega x)$型**

$$
(D^2+1)y = \sin x
$$

$$
\begin{aligned}
    y_p &= \frac{1}{D^2+1}\sin x\\
    &= \frac{1}{0}\sin x \\
    &= x\cdot \frac{1}{(D^2+1)'}\sin x\\
    &= x\cdot \frac{1}{2D}\sin x\\
    &=- \frac{x\cos x}{2}
\end{aligned}
$$

**e.g.2：$P_m(x)$型**
$$
(D^2-4D+4)y = x^2+3x +6
$$

$$
\begin{aligned}
    y_p &= \frac{1}{D^2-4D+4}(x^2+3x +6)\\
    &= \frac{1}{(D-2)^2}(x^2+3x +6)\\
    &= (\frac{1}{4} + \frac{1}{4}D+\frac{3}{16}D^2\cdots)(x^2+3x +6)\\
    &= \frac{1}{4}(x^2+3x +6)+ \frac{1}{4}(2x +3)+ \frac{3}{16}(2)\\
    &= \frac{1}{4}x^2 + \frac{5}{4}x + \frac{21}{8}
\end{aligned}
$$

**e.g.3：$P_m(x)e^{\lambda x}$型**
$$
(D^2+D+3)y = xe^x
$$

$$
\begin{aligned}
    y_p &= \frac{1}{D^2+D+3}(xe^x)\\
    &= e^x\cdot \frac{1}{(D+1)^2+(D+1)+3}x\\
    &= e^x\cdot \frac{1}{D^2+3D+5}x\\
    &= e^x\cdot (\frac{1}{5}-\frac{3}{25}D\cdots)x\\
    &= e^x(\frac{1}{5}x - \frac{3}{25} )\\
\end{aligned}
$$


**e.g.4：$P_m(x)\sin\omega x$型**
$$
(D^2+2D+5)y = e^{2x}\cos x
$$

$$
\begin{aligned}
    y_p &= \frac{1}{D^2+2D+5}(e^{2x}\cos x)\\
    &= e^{2x}\cdot \frac{1}{(D+2)^2+2(D+2)+5}(\cos x)\\
    &= e^{2x}\cdot \frac{1}{D^2+6D+13}(\cos x)\\
    &\stackrel{\omega^2 = -D^2 = 1}{\;\;=\;\;} e^{2x}\frac{1}{6D+12}\cos x\\
    &= \frac{e^{2x}}{6}\frac{1}{D+2}\cos x\\
    &= \frac{e^{2x}}{6}\frac{D -2}{(D+2)(D-2)}\cos x\\
    &= \frac{e^{2x}}{6}\frac{D-2}{-5}\cos x\\
    &=\frac{-1}{30}e^{2x}(D-2)\cos x\\
    &= \frac{-1}{30}e^{2x}(-\sin x + 2\cos x)\\
    &= \frac{1}{30}e^{2x}(\sin x + \cos x)
\end{aligned}
$$
