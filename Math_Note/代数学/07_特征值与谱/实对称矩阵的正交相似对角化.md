...
- 推导：
  ...
  3. 利用对称性与内积：
     $$
     v^{H}Av=\lambda\,v^{H}v,\qquad v^{H}Av=(Av)^{H}v=\overline{\lambda}\,v^{H}v.
     $$
  4. 得 $\lambda=\overline{\lambda}\in\mathbb R$。
- 意义：谱（特征值集合）在实数域闭合。
...
- 性质：存在正交矩阵 $Q$（$Q^\mathsf{T}Q=I$）与对角矩阵 $D=\operatorname{diag}(\lambda_1,\dots,\lambda_n)$，使
  $$
  A=QDQ^\mathsf{T}.
  $$
- 推导：
...
| 性质 | 描述 | 简要推导 |
| --- | --- | --- |
| 迹等于特征值之和 | $\operatorname{tr}(A)=\sum_i \lambda_i$ | $\operatorname{tr}(QDQ^\mathsf{T})=\operatorname{tr}(D)=\sum_i\lambda_i$（迹不变性） |
| 行列式等于特征值之积 | $\det(A)=\prod_i \lambda_i$ | $\det(QDQ^\mathsf{T})=\det(D)=\prod_i\lambda_i$（行列式乘性） |
| 谱范数与条件数 | $\lVert A\rVert_2=\max_i\lvert\lambda_i\rvert$；若 $A$ 可逆，$\kappa_2(A)=\dfrac{\max_i\lvert\lambda_i\rvert}{\min_i\lvert\lambda_i\rvert}$ | 正交相似不改变 2-范数；对角矩阵的谱范数由最大绝对特征值给出 |
| 不可逆判据 | $A$ 不可逆 $\Leftrightarrow 0$ 为特征值 | 由 $\det(A)=\prod_i\lambda_i$ 可知 |

备注：

- Rayleigh 商与极值：$\displaystyle \lambda_{\max}=\max_{\lVert x\rVert=1}x^\mathsf{T}Ax,\ \lambda_{\min}=\min_{\lVert x\rVert=1}x^\mathsf{T}Ax$。
- 若按不同特征值聚合为投影 $P_j$，则亦有投影谱分解 $A=\sum_j \mu_j P_j$、$P_j^2=P_j$、$P_iP_j=0\ (i\neq j)$、$\sum_j P_j=I$。

## 6. 相关笔记

- [对称矩阵](../09_特殊矩阵/对称矩阵.md)：结构层总览（自伴随本质、子空间、乘积与合同/相似行为、定号判定）
- [实对称矩阵的规范型](../08_二次型与合同/实对称矩阵的规范型.md)：Sylvester 惯性定理
