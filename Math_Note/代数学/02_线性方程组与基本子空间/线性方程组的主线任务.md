...
$$
**几何解释**：$\mathrm{Null}(A)$ 是一个二维平面（方向由两基向量给出），非齐次方程的解集是把它平移到经过特解点的位置所得的二维仿射平面。
...
**Step2**：$\operatorname{rank}(A)=2<n=3$，故 $\operatorname{nullity}(A)=1$，所以有无穷多解。由于 $m<n$，该矩阵不可能满列秩。
